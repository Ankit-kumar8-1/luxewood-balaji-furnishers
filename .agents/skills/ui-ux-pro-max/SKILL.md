...
- **Stack**: detect from the project — check `package.json` deps (react/next/vue/svelte/nuxt/@angular), `pubspec.yaml` (Flutter), `*.xcodeproj`/`Package.swift` (SwiftUI), `composer.json` (Laravel), or React Native markers (`app.json` + `react-native` dep). Default to `html-tailwind` if not specified.

### Step 2: Generate Design System

Determine color scheme, typography, responsive rules, accessibility guidelines, and component composition following the priority table above before building the interface.
